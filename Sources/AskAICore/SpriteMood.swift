...
public struct SpriteSet: Codable, Equatable, Sendable, Identifiable {
    ...
    ///
    /// A small owl in spectacles, generated by the app's own pipeline and cut
    /// with the shared-scale extractor — so the shipped character is an example
    /// of what the feature produces rather than a special case. Its source
    /// sheets are in `Resources/SpriteSources`, and `make sprites` re-cuts them.
    ///
    ...
    public static let builtIn = SpriteSet(
        ...
        name: "Owl",
        animations: [
            // Pose order is authored in the prompt: idle, explaining, puzzled,
            // searching.
            ...
            // The 6-frame ponder cycle. Slower than a walk: pondering at walking
            // speed reads as agitation, and this is the one loop the user
            // watches for several seconds while an answer loads.
            SpriteMood.thinking.rawValue:
                SpriteAnimation(
                    frames: (0..<6).map { "walk-\($0)" },
                    frameDuration: 0.18,
                    loops: true),
            // Gesture, then settle to attentive. The resting frame is the calm
            // pose, so Reduce Motion shows a character at rest rather than one
            // frozen mid-explanation.
            SpriteMood.talking.rawValue:
                SpriteAnimation(
                    frames: ["pose-1", "pose-0"],
                    frameDuration: 0.22,
                    loops: false),
