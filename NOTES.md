...
## The shipped character is now the owl

The guitarist is gone from the repo. `SpriteSet.builtIn` is a small owl in
spectacles, generated by the app's own pipeline, so the shipped character is an
example of what the feature produces rather than a hand-picked special case.
That also retires the last dependency on `~/Desktop/sprite-sheet-creator`, whose
assets the guitarist came from and one of whose sheets was a copyrighted
character.

Its source sheets are vendored in `Resources/SpriteSources` (628 KB) so
`make sprites` can re-cut them; verified byte-identical to what was committed.
The frames are therefore reproducible rather than opaque blobs.

`Options.vendored` no longer differs from the generated defaults. It existed to
reproduce the old hand-drawn art's 235 threshold and no-pixel-snapping; the
shipped character is now cut exactly like a user's, which removes a special case
rather than adding one.

The pose semantics changed with the character: `pose-0` is the attentive idle,
`pose-1` explaining, `pose-2` puzzled, `pose-3` searching — the order the current
prompt authors. The guitarist's frames meant crouch/leap/kneel/stand, which is
why its `talking` animation rested on `pose-3` and the owl's rests on `pose-0`.
